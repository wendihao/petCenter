<template>
  <div style="height:500px;">
    <el-table :data="tableData" border style="width: 100%;" >
      ...
      <el-table-column prop="username" label="用户账号" width="300">
      </el-table-column>
      ...
      <el-table-column prop="phoneid" label="用户号码" width="220">
      </el-table-column>
      <el-table-column prop="portrait" label="用户头像" width="200">
        <template slot-scope="scope">
          <img :src="scope.row.portrait" style="height: 100px;width: 100px;margin-left: 20%" />
        </template>
      </el-table-column>
          ...
          <el-dialog title="用户信息" :visible.sync="dialogFormVisible" :append-to-body="true">
            <el-form :model="form" ref="form">
              ...
              <el-form-item label="用户头像" :label-width="formLabelWidth">
                <el-image style="width: 100px; height: 100px" :src="form.portrait"/>
              </el-form-item>
            </el-form>
...
export default {
  ...
  methods: {
    ...
    getData (currentPage) {
      const _this = this
      this.$axios.get('http://119.29.250.12:8081/findAllUser/' + currentPage + '/4').then(function (resp) {
        console.log(resp)
    ...
    getUser (row) {
      ...
      const _this = this
      this.$axios.get('http://119.29.250.12:8081/user/selectbyid/' + row.id).then(function (resp) {
        _this.form = resp.data
      })
    },
    controlUser (row) {
      const _this = this
      this.$axios.get('http://119.29.250.12:8081/user/prohibit/' + row.id).then(function (resp) {
    ...
    controlUser1 (row) {
      const _this = this
      this.$axios.get('http://119.29.250.12:8081/user/jiefeng/' + row.id).then(function (resp) {
        if (resp.data === 'success') {
          _this.$message({
            showClose: true,
            type: 'success',
    ...
    deleteUser (row) {
      const _this = this
      this.$axios.get('http://119.29.250.12:8081/user/delete/' + row.id).then(function (resp) {
        if (resp.data === 'success') {
          _this.$alert('删除成功', {
            confirmButtonText: '确定',
            callback: action => {
