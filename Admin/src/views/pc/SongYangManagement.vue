    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column prop="images" label="上传图片" align="center" width="200">
        <template slot-scope="scope">
          <img :src="scope.row.images" style="height: 100px;width: 100px;margin-left: 20%" />
        </template>
      </el-table-column>
      <el-table-column prop="introduce" label="上传信息" width="400">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getData (currentPage) {
      const _this = this
      this.$axios.get('http://119.29.250.12:8081/findAllSongYang/' + currentPage + '/4').then(function (resp) {
        console.log(resp)
    ...
    getUpload (row) {
      ...
      const _this = this
      this.$axios.get('http://119.29.250.12:8081/findUserUpload/' + row.id).then(function (resp) {
        _this.form = resp.data
      })
    },
    update (row) {
      const _this = this
      this.$axios.get('http://119.29.250.12:8081/update/' + row.id).then(function (resp) {
        window.alert(row.id)
