...
export default {
  ...
  methods: {
    open (row) {
      this.$confirm('是否接受对方领养', '提示', {
        ...
        this.$axios.get('http://localhost:8081/sethomelesspet/' + row.homelesspetId + '/' + row.name).then(function (resp) {
          console.log(resp)
        })
        this.$message({
          type: 'success',
  ...
  created () {
    ...
    const _this = this
    this.$axios.get('http://localhost:8081/querry/allrenling/' + _this.$route.query.id).then(function (resp) {
