<template>
  <div style="display: flex;justify-content: space-around;">
    ...
    <div>
      <!--城市-->
      <div style="text-align: left;font-size: large">领养城市:</div>
      <div style="float: left">
        <el-input v-model="value" placeholder="输入城市名称"  indicator-position="outside" >
...
export default {
  ...
  methods: {
    ...
    handleCheckedCitiesChange (value) {
      const _this = this
      _this.value = value
      window.alert(_this.value)
      let checkedCount = value.length
      this.checkAll = checkedCount === this.cities.length
      this.isIndeterminate = checkedCount > 0 && checkedCount < this.cities.length
      this.$axios.get('http://localhost:8081/findByAddress1/' + _this.currentPage + '/4/' + value).then(function (resp) {
    ...
    getData (currentPage) {
      const _this = this
      this.$axios.get('http://localhost:8081/findAll/' + currentPage + '/4').then(function (resp) {
        _this.blogs = resp.data.records
        for (let i = 0; i < resp.data.records.length; i++) {
          _this.blogs[i].images = resp.data.records[i].images.split(';')
        }
