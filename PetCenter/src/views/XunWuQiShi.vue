  ...
  <div style="display: flex;justify-content: space-around;">
    ...
    <div>
      ...
      <div>
        ...
        <div v-for="xunwu in XunWus" style="width: 600px;">
          <div style="display:flex;background-color: white;padding: 10px;margin-bottom: 10px;">
            ...
            <div>
              ...
              <div style="text-align: left;;margin-top: -15px;color: slategray;" @click="blogdetails(xunwu.id,xunwu.userId)" class="XunWu-title"><h4>{{寻物启事}}</h4></div>
              ...
              <div style="text-align: initial;margin-top: -25px;"><p style="font-size: small;color: #999;font-size: medium;" class="p-overflow">{{xunwu.introduce}}</p></div>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleCheckedCitiesChange (value) {
      ...
      this.$axios.get('http://localhost:8081/findByAddress2/' + _this.currentPage + '/4/' + value).then(function (resp) {
    ...
    getData (currentPage) {
      const _this = this
      this.$axios.get('http://localhost:8081/findAllXunWu/' + currentPage + '/4').then(function (resp) {
        _this.XunWus = resp.data.records
        for (let i = 0; i < resp.data.records.length; i++) {
          window.alert(_this.XunWus[i].address)
          _this.XunWus[i].images = resp.data.records[i].images.split(';')
        }
        _this.total = resp.data.total
        console.log(resp)
