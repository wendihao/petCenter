  ...
  <div class="tmpl">
    <ul class="mui-table-view">
      <li class="mui-table-view-cell mui-media" v-for="(item,index) in newsList" :key="index">
        <router-link :to="{name:'XiangQing1M',query:{id:item.id,userId:item.userId,renlingId:renlingId,status:status}}">
...
export default{
  ...
  created () {
    const _this = this
    this.$axios.get('http://119.29.250.12:8081/index/select/' + '宠物招领').then(function (resp) {
    ...
    _this.renlingId = _this.$route.query.id
  }
}
</script>
