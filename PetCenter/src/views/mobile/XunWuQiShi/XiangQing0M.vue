...
<!--          <el-breadcrumb-item ><a @click=" removeIndex()" class="blog-title">首页</a></el-breadcrumb-item>-->
      ...
      <div style="background-color: beige;padding: 0.2rem;">
        <div style="display: flex;justify-content: space-around;">
          <div style="width: 55%;height: 100%">
            <el-carousel style="height: 100%">
              <el-carousel-item v-for="(image,index) in xunwu.images" :key="index" style=" height: 100%">
          ...
          <div class="HomeLessPet-message">
            <div >
              <p style="font-size: 14px"><i class="el-icon-time blog-icon"></i>丢失时间: {{xunwu.datetime}}</p>
            </div>
            <div>
              <p style="font-size: 14px"><i class="el-icon-location-outline blog-icon"></i>丢失地点: {{xunwu.address}}</p>
            </div>
            <div>
              <p style="font-size: 14px"><i class="el-icon-user blog-icon"></i>联系人： {{xunwu_user.name}}</p>
            </div>
            <div >
              <p style="font-size: 14px"><i class="el-icon-phone blog-icon"></i>联系电话: {{xunwu_user.phoneid}}</p>
            </div>
          </div>
        </div>
        <div style="margin-top: 0.5rem">
          ...
          <div>
            <el-button v-if="xunwu_user.id!=user.id" type="success" style="margin-left: 40%" @click="removeim" round>联系</el-button>
          </div>
        </div>
      </div>
    </div>
