<template>
  <div class="tmpl" style="width: 100% ;height:120%">
    <div style="background-color: white;padding: 0.1rem;width: 100%">
      ...
      <h1 style="margin-left: 40%;margin-top: -5%;font-size: 15px;font-weight: bolder">我的收养</h1>
      <hr style="margin-bottom: -1%;width: 100%">
    </div>
    <ul class="mui-table-view">
      <li class="mui-table-view-cell mui-media" v-for="(item,index) in blogs" :key="index">
        <router-link :to="{name:'XiangQing2M',query:{id:item.id,userId:item.userId,adopterId:adopterId,status:item.status}}">
...
export default{
  ...
  created () {
    const _this = this
    this.$axios.get('http://119.29.250.12:8081/findmyadoption/' + _this.$route.query.id).then(function (resp) {
      _this.blogs = []
