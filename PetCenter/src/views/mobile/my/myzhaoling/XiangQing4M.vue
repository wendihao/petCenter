      ...
      <div style="background-color: beige;padding: 0.2rem;">
        <div style="display: flex;justify-content: space-around;">
          ...
          <div class="blog-message">
            ...
            <div>
              <p style="font-size:  14px"><i class="el-icon-time blog-icon"></i>认领时间: {{renling.datetime}}</p>
            </div>
            <div>
              <p style="font-size: 14px"><i class="el-icon-location-outline blog-icon"></i>地点: {{homelesspet.address}}</p>
            </div>
            <div>
              <p style="font-size:  14px"><i class="el-icon-user blog-icon"></i>认领人： {{homelesspet_user.name}}</p>
            </div>
            <div>
              <p style="font-size:  10px"><i class="el-icon-phone blog-icon"></i>联系电话: {{homelesspet_user.phoneid}}</p>
            </div>
            <div v-if="this.$route.query.schedule==0">
              <p style="font-size:  14px">审核中....</p>
            </div>
          </div>
        </div>
        <div v-if="this.$route.query.schedule==1&&this.$route.query.renlingId!=null" style="margin-top: 1rem">
          <el-divider content-position="left"><h3>认领描述</h3></el-divider>
          <div style="text-align: start;">
            <p>{{renling.condit}}</p>
          </div>
        </div>
        <div v-if="this.$route.query.schedule==0" style="margin-top: 1rem">
