      ...
      <div>
        ...
        <div v-for="homelesspet in homelesspets" style="width: 600px;">
          <div style="display:flex;background-color: white;padding: 10px;margin-bottom: 10px;">
            ...
            <div>
              <div style="display: none;">{{homelesspet.id}}</div>
              <div style="text-align: left;;margin-top: -15px;color: slategray;" @click="blogdetails(homelesspet.id,homelesspet.userId)" class="blog-title"><h4>{{homelesspet.title}}</h4></div>
...
export default {
  ...
  methods: {
    ...
    getData (currentPage) {
      const _this = this
      this.$axios.get('http://localhost:8081/findByUserId/' + currentPage + '/4/' + _this.$route.query.id).then(function (resp) {
