<template>
  <div style="height:500px;">
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column prop="introduce" label="上传内容" width="200">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getData (currentPage) {
      const _this = this
      this.$axios.get('http://localhost:8081/findAllPetBlog/' + currentPage + '/4').then(function (resp) {
        console.log(resp)
    ...
    deletePetCircle (row) {
      const _this = this
      this.$axios.get('http://localhost:8081/deletePetCircle/' + row.id).then(function (resp) {
