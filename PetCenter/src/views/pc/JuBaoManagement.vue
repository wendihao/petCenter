<template>
  <div style="height:500px;">
    <el-table :data="tableData" border style="width: 100%">
      <el-table-column prop="jubaoId" label="举报人ID" width="100">
      </el-table-column>
      <el-table-column prop="jubaousername" label="举报人账号" width="100">
      </el-table-column>
      <el-table-column prop="username" label="被举报人账号" width="100">
      </el-table-column>
      <el-table-column prop="reason" label="举报原因" width="200">
      </el-table-column>
      <el-table-column prop="images" label="相关截图" align="center" width="200">
...
export default {
  ...
  methods: {
    ...
    getData (currentPage) {
      const _this = this
      this.$axios.get('http://localhost:8081/findAllJuBao/' + currentPage + '/4').then(function (resp) {
        console.log(resp)
    ...
    getUpload (row) {
      ...
      const _this = this
      this.$axios.get('http://localhost:8081/findJuBaoById/' + row.id).then(function (resp) {
        _this.form = resp.data
      })
    },
    deleteJuBao (row) {
      const _this = this
      this.$axios.get('http://localhost:8081/deleteJuBao/' + row.id).then(function (resp) {
    ...
    sendMessage (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$axios.post('http://localhost:8081/sendMessage1', _this.form).then(function (resp) {
            if (resp.data === 'success') {
              _this.$alert('发送成功', {
                confirmButtonText: '确定',
                callback: action => {
