        ...
        <div style="display: flex;justify-content: space-around;">
          <div style="width: 55%;height: 400px">
            <el-carousel style="height: 400px">
              <el-carousel-item v-for="(image,index) in homelesspet.images" :key="index" style=" height: 400px">
          ...
          <div class="blog-message">
            <div>
              <p style="font-size: 20px"><i class="el-icon-time blog-icon"></i>发布时间: {{homelesspet.datetime}}</p>
            </div>
            <div>
              <p style="font-size: 20px"><i class="el-icon-time blog-icon"></i>认领时间: {{renling.datetime}}</p>
            </div>
            <div>
              <p style="font-size: 20px"><i class="el-icon-location-outline blog-icon"></i>地点: {{homelesspet.address}}</p>
            </div>
            <div>
              <p style="font-size: 20px"><i class="el-icon-user blog-icon"></i>认领人： {{homelesspet_user.name}}</p>
            </div>
            <div>
              <p style="font-size: 20px"><i class="el-icon-phone blog-icon"></i>联系电话: {{homelesspet_user.phoneid}}</p>
            </div>
            <div v-if="this.$route.query.schedule==0">
              <p style="font-size: 20px">审核中....</p>
            </div>
          </div>
        </div>
...
export default {
  ...
  created () {
    ...
    this.$axios.get('http://119.29.250.12:8081/user/selectbyid/' + _this.$route.query.renlingId).then(function (resp) {
