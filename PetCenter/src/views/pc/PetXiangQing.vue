...
<!--        </div>-->
      ...
      <!--标题-->
      <div style="text-align: initial;margin-top: -15px;margin-left: 15px">
        <h2>{{petBlog.title}}</h2>
      </div>
      <!--博主信息-->
      <div class="blog-user">
      ...
      <div>
        ...
        <div id="div-nav" style="margin-top: 30px;height: 300px;overflow: auto">
          <div style="float: left;width: 100%" class="listItem" v-for="item in commList" :key="item">
            <div>
              <div  style="float: left;margin-top: 1rem" class="userInfo">
                <img :src="item.portrait" style="width: 50px;height: 50px;float: left" alt="">
                <p style="font-size: 15px;float: left;padding-top: 2rem">{{item.username}}:{{item.comment}}</p>
...
export default {
  ...
  created () {
    ...
    this.$axios.get('http://119.29.250.12:8081/user/select/' + _this.$route.query.username).then(function (resp) {
      ...
      _this.comment.user_id = resp.data.id
    })
