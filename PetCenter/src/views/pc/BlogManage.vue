...
export default {
  ...
  methods: {
    ...
    getData (currentPage, userid) {
      const _this = this
      this.$axios.get('http://119.29.250.12:8081/findAll/' + currentPage + '/5' + '/' + _this.$route.query.id).then(function (resp) {
        console.log(resp)
    ...
    getBlog (row) {
      ...
      const _this = this
      this.$axios.get('http://119.29.250.12:8081/findBlog/' + row.id).then(function (resp) {
    ...
    updateBlog (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$axios.post('http://119.29.250.12:8081/blog/update', _this.form).then(function (resp) {
    ...
    deleteBlog (row) {
      const _this = this
      this.$axios.delete('http://119.29.250.12:8081/blog/delete/' + row.id).then(function (resp) {
        if (resp.data === 'success') {
