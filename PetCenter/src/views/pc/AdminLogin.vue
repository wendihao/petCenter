...
export default {
  ...
  methods: {
    onSubmit (formName) {
      const _this = this
      this.$axios.get('http://localhost:8081/user/select/' + this.ruleForm.username).then(function (resp) {
        _this.ruleForm.id = resp.data.id
      })
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$axios.get('http://localhost:8081/adminlogin/' + _this.ruleForm.username + '/' + _this.ruleForm.password).then(function (resp) {
            console.log(resp)
            if (resp.data === 200) {
